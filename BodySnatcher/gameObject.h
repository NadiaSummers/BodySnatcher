...
class gameObject
{
	...
	public:
		...
		gameObject();
			...
			* @param string modelID - the model this game object uses
			*/
		gameObject(vector3D location, float obScale, float sSize, string modelID);
			...
			* @param string modelID - the model this game object uses
			*/
		gameObject(float x, float y, float z, float obScale, float sSize, string modelID);
		...
		void setScale(float obScale);
			...
			/*
			* @brief set the ID of the model the object uses
			* @param string modelID - the ID of the model the object uses
			* @return void
			*/
		...
		void setPositionZ(float z);
			...
			/*
			* @brief sets the z position of the object
			* @return string - returns the ID of the model the object uses
			*/
		string getModelID();
	...
	private:
		unsigned int mID; //the object's ID
		vector3D position;           //position vector
		float scale;                //scaling factor
		float sphereSize;			//size of bounding sphere
		string mModelID;			//the name of the model this object uses
		static unsigned int numObjects; //track the number of game objects
};
